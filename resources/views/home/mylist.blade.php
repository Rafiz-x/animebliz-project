...
@section('main')
    <section class="h-fit mx-auto w-full px-[3vw] pt-10 flex flex-col gap-5">
        ...
        <div
            ...
            @if ($notLogged)
                <h1 class="text-white flex items-center justify-center col-span-12 text-xl lg:text-2xl xl:text-2xl h-[10vh]">Please Login to Access this Page</h1>
            @else
                @if ($posts->isEmpty())
                    <h1
                        class="text-white flex items-center justify-center col-span-12 text-xl lg:text-2xl xl:text-2xl h-[10vh]">
                        No Post Found</h1>
                @else
                    @foreach ($posts as $post)
                        <div
                            ...
                            <a href="{{ route('post', $post->slug) }}" class="content w-full h-full group">
                                ...
                                <div
                                    class="info absolute bottom-[15%] left-[50%] -translate-x-[50%] text-[12px] sm:text-sm lg:text-base xl:text-lg text-white font-normal text-center pointer-events-none transition-all opacity-0 w-full flex !flex-col break-words px-0.5 group-hover:opacity-100">
                                    ...
                                    @endphp
                                    <span class="season font-semibold text-[#fd8817]">{{ $postYear }}</span>
