...
@section('content')
    ...
    <div class="edit_category w-full sm:max-w-96 md:max-w-[450px] lg:max-w-[600px] xl:max-w-[800px] mx-auto">
    @if ($categoryFound)
        <div class="form w-full">
            <form action="{{ route('admin.category.edit.handle') }}" method="POST" class="flex flex-col gap-8">
                @csrf

                <input type="text" name="category_id" id="category_id" value="{{ $currentCategory->id }}" onchange="window.location.reload()" hidden>

                <div class="field text-white flex flex-col gap-2">
                    <label for="name" class="lg:text-lg xl:text-xl">Name</label>
                    <input class="w-full" type="text" name="category_name" id="category_name" value="{{ $currentCategory->name }}" placeholder="Choose a Category Name"
                        autocomplete="off">
                ...
                <div class="field text-white flex flex-col gap-2">
                    ...
                    <input type="text" name="category_slug" id="category_slug" value="{{ $currentCategory->slug }}" placeholder="e.g. animation, fantasy">
                ...
                <div class="existing_categorys">
                    <ol
                        ...
                        @foreach ($categories as $name => $slug)
                            <li><a href="{{ route('admin.category.edit.withSlug', $slug) }}" class="hover:underline">{{ $name }}</a></li>
                        @endforeach
                    </ol>
                    <div class="info mt-2 flex gap-2 text-amber-500 text-sm">
                        ...
                        <p>Make sure your category name doesn't match any of those!</p>
                    </div>
                </div>
