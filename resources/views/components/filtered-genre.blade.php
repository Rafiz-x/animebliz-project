    ...
    <thead class="bg-gray-700">
        <tr>
            <th class="p-4 text-left">
                <input type="checkbox" name="select_all" id="select_all_genre" class="rounded">
            </th>
        ...
        @foreach ($genres as $genre)
            <tr class="border-t border-gray-700">
                <td class="p-4 text-left">
                    <input type="checkbox" name="checkbox_genre_{{ $genre->id }}" class="rounded">
                    <input type="text" name="{{ $genre->slug }}" hidden>
                </td>
                <td class="p-4 text-left">{{ $genre->id }}</td>
                <td class="p-4 text-left">{{ $genre->name }}</td>
                <td class="p-4 text-left">{{ $genre->slug }}</td>
                <td class="p-4 text-left">{{ $genre->created_at }}</td>
                <td class="p-4 text-left">{{ $genre->updated_at }}</td>
                <td class="p-4 text-left">
                    <a href="{{ route('genre', $genre->slug) }}" target="_blank"
                ...
                <td class="p-4 text-left">
                    <a href="{{ route('admin.genre.edit.withSlug', $genre->slug) }}"
