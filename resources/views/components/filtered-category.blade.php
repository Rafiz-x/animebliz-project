    ...
    <thead class="bg-gray-700">
        <tr>
            <th class="p-4 text-left">
                <input type="checkbox" name="select_all" id="select_all_category" class="rounded size-5 cursor-pointer">
            </th>
        ...
        @foreach ($categories as $category)
            <tr class="border-t border-gray-700">
                <td class="p-4 text-left">
                    <input type="checkbox" name="checkbox_category_{{ $category->id }}" class="rounded cursor-pointer size-5">
                    <input type="text" name="{{ $category->slug }}" hidden>
                </td>
                <td class="p-4 text-left">{{ $category->id }}</td>
                <td class="p-4 text-left">{{ $category->name }}</td>
                <td class="p-4 text-left">{{ $category->slug }}</td>
                <td class="p-4 text-left">{{ $category->created_at }}</td>
                <td class="p-4 text-left">{{ $category->updated_at }}</td>
                <td class="p-4 text-left">
                ...
                <td class="p-4 text-left">
                    <a href="{{ route('admin.category.edit.withSlug', $category->slug) }}"
